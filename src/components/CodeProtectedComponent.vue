<template>
  <div>
    ...
    <div v-if="!isCodeValid" class="code-input-container">
      ...
      <button @click="validateCode" class="submit-button">אמת קוד</button>
    </div>
...
.code-input-container {
  ...
  max-width: 400px; /* רוחב מקסימלי לתיבה */
  margin: 0 auto; /* מיקום במרכז אופקי */
  ...
  top: 50%; /* מיקום אנכי */
  left: 50%; /* מיקום אופקי */
  transform: translate(-50%, -50%); /* הזזה כדי למרכז */
}
...
.code-input-container input {
  ...
  box-sizing: border-box; /* כולל ריווח פנימי במסגרת */
}

.code-input-container input:focus {
  outline: none; /* הסרת מסגרת ברירת מחדל */
  border-color: #007bff; /* צבע מסגרת בעת פוקוס */
  box-shadow: 0 0 5px rgba(0, 123, 255, 0.5); /* צל בעת פוקוס */
}

.error-message {
  color: red; /* צבע טקסט אדום */
  font-size: 14px; /* גודל טקסט */
  margin-top: -5px; /* ריווח מעל ההודעה */
}
...
.submit-button {
  ...
  font-size: 16px; /* גודל טקסט */
  transition: background-color 0.3s ease, transform 0.2s ease; /* אפקט מעבר */
}
