...
The client reads the server address from `VUE_APP_API_BASE_URL`.

- Local Docker / same-origin proxy: leave it unset and the app will use `/api`
- Remote server example: `VUE_APP_API_BASE_URL=https://api.example.com`
- Remote server with path prefix: `VUE_APP_API_BASE_URL=https://api.example.com/api`

You can copy `.env.example` to `.env.local` for local development.

## Vercel deployment

- Set `VUE_APP_API_BASE_URL` in the Vercel project environment variables
